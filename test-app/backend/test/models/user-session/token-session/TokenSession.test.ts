...
import { ITokenPayload, TokenSessionParser, TokenSessionParserError } from '../../../../src/models/user-session/token-session/TokenSession';
...
describe('TokenSessionParser', () => {
  ...
  it('should throw an error when an authorization header is incorrectly formatted', async () => {
    ...
    try {
      ...
      expect(e).toStrictEqual(new TokenSessionParserError(TokenSessionParserError.BAD_BEARER_TOKEN));
    }
  ...
  it('should throw an error when token retrieval fails', async () => {
    ...
    try {
      ...
      expect(e).toStrictEqual(new TokenSessionParserError(TokenSessionParserError.RETRIEVAL_FAILED));
    }
