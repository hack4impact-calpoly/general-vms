...
function setupQueryWithResolver<T>({ resolver, ...setupQueryArgs }: ISetupQueryWithResolverArgs<T>): Query<T> {
  const query = setupQuery(setupQueryArgs);
  const originalQueryExecute = query.execute.bind(null);
  const newQueryFn = () => new Promise<T>((resolve, reject) => {
...
function HttpRequestDataLoader<T>({ dataLoaderProps, httpOpts, responseResolver }: IProps<T>) {
  ...
  return (
    <DataLoader
      query={query}
      errorComponent={
        <HttpRequestDataLoaderErrorComponent error={error} />
      }
      {...dataLoaderProps}
    />
  );
