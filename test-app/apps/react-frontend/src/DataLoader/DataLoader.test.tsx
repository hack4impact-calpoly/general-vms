...
describe("DataLoader component", () => {
  ...
  it("should pass through addtional props to component when promise is resolved", () => {
    render(
      <DataLoader
        OnceLoadedComponent={({ data, more }: { data: string; more: string }) => (
          <p>
            {data} + {more}
          </p>
        )}
  ...
  it("should show custom loading component when specified until promise resolves", async () => {
    ...
    return waitFor(() => expect(screen.queryByText(text)).toBeFalsy(), { timeout: 200 });
  });
  ...
  it("should show default error component upon error and no error component is specified", () => {
    ...
    return waitFor(() => {
      // Check alert
      expect(
        screen.getByText("An error occurred while attempting to load data", { exact: false }),
      ).toBeTruthy();
      // Check popup
