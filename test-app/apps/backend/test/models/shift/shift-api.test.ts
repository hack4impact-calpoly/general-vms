...
import { getShallowCopyWithoutProps } from "../../test-utils";
import { runValidationTest } from "../../test-utils/validation";
  ...
  describe("POST /api/new-shift", () => {
    ...
    test("Shift fails to create because there is no title", async () => {
      await runValidationTest({
        ...
        expectedErrors: [{ path: ["title"], message: "Required" }],
      });
    });
    ...
    test("Shift fails because start date is after end date", async () => {
      await runValidationTest({
        ...
        expectedErrors: [{ path: [], message: "start date must come before end date" }],
      });
    });
