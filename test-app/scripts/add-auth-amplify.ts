import { removeMultipleContiguousSpaces, runCommand, setup, wrapAsyncCall } from './shared-amplify-config';
...
async function addAuthAmplify() {
  ...
  const pascalCaseName = hyphenatedAppName.split('-').map(word => word[0].toUpperCase() + word.slice(1)).join('');
